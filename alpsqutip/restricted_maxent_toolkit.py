...
def build_Hierarch(generator, seed_op, deep):
    ...
    if seed_op is not None and deep > 0:
        ...
        for k in range(deep):
            ...
            basis.append(commutator(1j * generator.to_qutip(), basis[-1]))
    return basis
...
def orthogonalize_basis(basis, sp: callable, tol=1e-5):
    ...
    orth_basis = [
        sum(linalg.sqrtm(inv_gram_matrix)[j][i] * local_basis[j] for j in range(len(local_basis)))
    ...
    assert (
        linalg.norm(gram_matrix(basis=orth_basis, sp=sp) - np.identity(len(orth_basis))) < tol
